...
$(function () {
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['semester_name'];
            }
          }),
          type: 'column',
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = '/app/settings/semester';
      var method = 'POST';
      var message = 'Semester added successfully.';
      var data = {
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    if (confirm('Are you sure you want to delete this record?')) {
        $.ajax({
            url: '/app/settings/semester/' + data.id,
            type: 'DELETE',
            data: {
                _token: $('meta[name="csrf-token"]').attr('content')
            },
  ...
  $('.datatables-basic tbody').on('click', '.item-edit', function () {
    ...
    document.querySelector('.dt-full-name').value = data.semester_name;
