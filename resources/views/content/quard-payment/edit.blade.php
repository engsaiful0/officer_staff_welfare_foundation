...
@section('title', 'Quard Payments - Edit')
  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Edit Quard Payment</h5>
      ...
      <div class="card-body">
        <form action="{{ route('quard-payment.update', $quardPayment->id) }}" method="POST" id="quardPaymentForm" novalidate>
          @csrf
          @method('PUT')
          ...
          <div class="row">
            <div class="col-md-4 mb-3">
              ...
              <select class="select2 form-select @error('member_id') is-invalid @enderror"
                      id="member_id"
                      name="member_id"
                      required>
                @foreach($members as $m)
                  <option value="{{ $m->id }}" {{ (string) old('member_id', $quardPayment->member_id) === (string) $m->id ? 'selected' : '' }}>
                    {{ $m->name }} ({{ $m->unique_id }})
                  </option>
                @endforeach
              </select>
            ...
            <input type="hidden" id="quard_id" name="quard_id" value="{{ old('quard_id', $quardPayment->quard_id) }}" required>
            ...
            <div class="col-md-4 mb-3">
              ...
              <div class="input-group">
                ...
                <input type="number"
                       ...
                       id="payment_amount"
                       name="payment_amount"
                       value="{{ old('payment_amount', $quardPayment->payment_amount) }}"
                       step="0.01"
                       min="0"
                       readonly
                       required>
              </div>
              <div class="form-text">Auto-populated from selected member’s latest quard.</div>
            ...
            <div class="col-md-4 mb-3">
              ...
              <input type="date"
                     ...
                     id="payment_date"
                     name="payment_date"
                     value="{{ old('payment_date', $quardPayment->payment_date ? $quardPayment->payment_date->format('Y-m-d') : '') }}"
                     required>
              @error('payment_date')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            ...
            <div class="col-4 mb-3">
              ...
              <textarea class="form-control @error('notes') is-invalid @enderror"
                        ...
                        name="notes"
                        rows="3"
                        placeholder="Notes">{{ old('notes', $quardPayment->notes) }}</textarea>
              @error('notes')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
            ...
            <div class="col-12">
              <div class="d-flex gap-2">
                <button type="button" class="btn btn-primary" id="submitBtn">
                  ...
                  <span id="submitText">Update Quard Payment</span>
                </button>
...
jQuery(document).ready(function($) {
  ...
  // On page load, we keep the existing payment_amount/quard_id as-is.
  ...
  function resetSubmitButton() {
    ...
    $('#submitText').text('Update Quard Payment');
  }
  ...
  function submitFormViaAjax() {
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.success) {
          if (typeof toastr !== 'undefined') toastr.success(response.message || 'Quard payment updated successfully');
          else alert(response.message || 'Quard payment updated successfully');

          setTimeout(function() {
            window.location.href = '{{ route("quard-payment.view-quard-payment") }}';
          }, 800);
        } else {
          if (typeof toastr !== 'undefined') toastr.error(response.message || 'Failed to update quard payment');
          else alert(response.message || 'Failed to update quard payment');
          resetSubmitButton();
        }
      },
      error: function(xhr) {
        ...
        if (xhr.status === 422 && responseJSON && responseJSON.errors) {
          ...
          const errorMessage = (responseJSON && responseJSON.message) ? responseJSON.message : 'An error occurred while updating the quard payment.';
