...
@section('title', 'HPSM — Edit account')

@section('content')
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
    <h5 class="mb-0">Edit {{ $account->account_no }}</h5>
    <a href="{{ route('hpsm-opening-accounts.show', $account) }}" class="btn btn-outline-secondary btn-sm">Back</a>
  </div>
  <div class="card-body">
    @if($errors->any())
      <div class="alert alert-danger"><ul class="mb-0">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></div>
    @endif
    <form method="post" action="{{ route('hpsm-opening-accounts.update', $account) }}">
      @csrf
      @method('PUT')
      @include('hpsm_opening_accounts._form', ['account' => $account, 'members' => $members, 'mayReschedule' => $mayReschedule])
...
(function(){
  ...
  ['balance_principal','balance_pre_rent','current_rent','annual_profit_rate','remaining_duration_months'].forEach(function(id){
    var el = document.getElementById(id); if(el && !el.readOnly && !el.disabled) el.addEventListener('input', recalc);
  });
  recalc();
})();
</script>
