...
$(function () {
  ...
  if (typeof window.boardUrls === 'undefined') {
    console.error('Board URLs not defined');
    return;
  }
  ...
  if (dt_basic_table.length) {
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['board_name'];
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Boards</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.boardUrls.store;
      var method = 'POST';
      var message = 'Board added successfully.';
      var data = {
      ...
      if (id) {
        url = window.boardUrls.update + '/' + id;
        method = 'PUT';
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: window.boardUrls.destroy + '/' + data.id,
                type: 'DELETE',
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content')
                },
  ...
  $('.datatables-basic tbody').on('click', '.item-edit', function () {
    ...
    document.querySelector('.dt-full-name').value = data.board_name;
