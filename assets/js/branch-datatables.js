...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ajax: {
        url: AppUtils.buildUrl('app/settings/get-branch'),
        type: 'GET',
        dataSrc: 'data',
        beforeSend: function() {
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = AppUtils.buildUrl('app/settings/branch');
      var method = 'POST';
      ...
      if (id) {
        url = AppUtils.buildUrl('app/settings/branch/' + id);
        method = 'PUT';
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          url: AppUtils.buildUrl('app/settings/branch/' + data.id),
          type: 'DELETE',
          data: {
            _token: $('meta[name="csrf-token"]').attr('content')
          },
