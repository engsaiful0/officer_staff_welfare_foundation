...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ajax: {
        url: window.incomeHeadUrls.getData,
        type: 'GET',
        dataSrc: 'data'
      },
      columns: [
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.incomeHeadUrls.store;
      var method = 'POST';
      ...
      if (id) {
        url = window.incomeHeadUrls.update + '/' + id;
        method = 'PUT';
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: window.incomeHeadUrls.destroy + '/' + data.id,
                type: 'DELETE',
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content')
                },
                success: function (response) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Deleted!',
                        text: 'Income Head has been deleted.',
                        customClass: {
