    ...
    fv = FormValidation.formValidation(formAddNewRecord, {
      fields: {
        nationality_name: {
          validators: {
            notEmpty: {
              message: 'The nationality name is required'
            }
          }
        }
      },
...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ajax: {
        url: window.nationalityUrls.getData,
        type: 'GET',
        dataSrc: 'data'
      },
      columns: [
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['nationality_name'];
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Nationality Name</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.nationalityUrls.store;
      var method = 'POST';
      var message = 'Nationality added successfully.';
      var data = { nationality_name: $new_name };

      if (id) {
        url = window.nationalityUrls.update + '/' + id;
        method = 'PUT';
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
            url: window.nationalityUrls.destroy + '/' + data.id,
            type: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
  ...
  $('.datatables-basic tbody').on('click', '.item-edit', function () {
    ...
    document.querySelector('.dt-full-name').value = data.nationality_name;
