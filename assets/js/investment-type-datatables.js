/**
 * DataTables Basic for Investment Type
 */
    ...
    fv = FormValidation.formValidation(formAddNewRecord, {
      fields: {
        investment_type_name: {
          validators: {
            notEmpty: {
              message: 'The investment type name is required'
            }
          }
        }
      },
...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ajax: {
        url: AppUtils.buildUrl('app/settings/get-investment-type'),
        type: 'GET',
        dataSrc: 'data',
        beforeSend: function() {
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['investment_type_name'];
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Investment Types</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = AppUtils.buildUrl('app/settings/investment-type');
      var method = 'POST';
      var message = 'Investment type added successfully.';
      var data = {
        _token: $('meta[name="csrf-token"]').attr('content'),
        investment_type_name: $new_name
      };

      if (id) {
        url = AppUtils.buildUrl('app/settings/investment-type/' + id);
        method = 'PUT';
      ...
      $.ajax({
        ...
        error: function (error) {
          ...
          if (error.responseJSON && error.responseJSON.errors) {
            ...
            if (errors.investment_type_name) {
              errorMessage = errors.investment_type_name[0];
            }
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          url: AppUtils.buildUrl('app/settings/investment-type/' + data.id),
          type: 'DELETE',
          data: {
            _token: $('meta[name="csrf-token"]').attr('content')
          },
          success: function (response) {
            ...
            setTimeout(function() {
              row.remove().draw();
              toastr.success("Investment type has been deleted.");
            }, 300);
          },
          error: function (error) {
            if (error.responseJSON && error.responseJSON.message) {
              ...
              toastr.error('An error occurred while deleting the investment type.');
            }
            console.log(error);
          },
          complete: function() {
  ...
  $('.datatables-basic tbody').on('click', '.item-edit', function () {
    ...
    document.querySelector('.dt-full-name').value = data.investment_type_name;
    $('#form-add-new-record').attr('data-id', data.id);
    document.querySelector('#exampleModalLabel').textContent = 'Edit Investment Type';
    offCanvasEl.show();
  });
  ...
  $('.create-new').on('click', function() {
    setTimeout(function() {
      document.querySelector('#exampleModalLabel').textContent = 'New Investment Type';
    }, 100);
  });
});
