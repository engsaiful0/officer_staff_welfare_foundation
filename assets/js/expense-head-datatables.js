    ...
    fv = FormValidation.formValidation(formAddNewRecord, {
      fields: {
        name: {
          validators: {
            notEmpty: {
              message: 'The expense head name is required'
            }
          }
        }
      },
...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ajax: {
        url: window.expenseHeadUrls.getData,
        type: 'GET',
        dataSrc: 'data'
      },
      columns: [
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Expense Heads</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.expenseHeadUrls.store;
      var method = 'POST';
      var message = 'Expense Head added successfully.';
      var data = {
      ...
      if (id) {
        url = window.expenseHeadUrls.update + '/' + id;
        method = 'PUT';
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: window.expenseHeadUrls.destroy + '/' + data.id,
          type: 'DELETE',
          data: {
            _token: $('meta[name="csrf-token"]').attr('content')
          },
