...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ajax: {
        url: window.religionUrls.getData,
        type: 'GET',
        dataSrc: 'data'
      },
      columns: [
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['religion_name'];
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Religion Name</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.religionUrls.store;
      var method = 'POST';
      var message = 'Religion added successfully.';
      var data = { religion_name: $new_name };

      if (id) {
        url = window.religionUrls.update + '/' + id;
        method = 'PUT';
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
            url: window.religionUrls.destroy + '/' + data.id,
            type: 'DELETE',
            success: function (response) {
                row.remove().draw();
  ...
  $('.datatables-basic tbody').on('click', '.item-edit', function () {
    ...
    document.querySelector('.dt-full-name').value = data.religion_name;
