...
$(function () {
  ...
  if (typeof window.academicYearUrls === 'undefined') {
    console.error('Academic Year URLs not defined');
    return;
  }
  ...
  if (dt_basic_table.length) {
    console.log('Using URL:', window.academicYearUrls.getData);
    dt_basic = dt_basic_table.DataTable({
      ajax: {
        url: window.academicYearUrls.getData,
        type: 'GET',
        dataSrc: 'data'
      },
      columns: [
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['academic_year_name'];
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Academic Year</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.academicYearUrls.store;
      var method = 'POST';
      var message = 'Academic Year added successfully.';
      var data = {
        _token: $('meta[name="csrf-token"]').attr('content'),
        academic_year_name: $new_name
      };

      if (id) {
        url = window.academicYearUrls.update + '/' + id;
        method = 'PUT';
      ...
      $.ajax({
        ...
        error: function (error) {
          if (error.responseJSON && error.responseJSON.errors) {
            ...
            AppUtils.showError(errorMessages);
          } else {
            AppUtils.showError('An error occurred.');
          }
        }
      });
    }
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: window.academicYearUrls.destroy + '/' + data.id,
          type: 'DELETE',
          data: {
            _token: $('meta[name="csrf-token"]').attr('content')
          },
  ...
  $('.datatables-basic tbody').on('click', '.item-edit', function () {
    ...
    document.querySelector('.dt-full-name').value = data.academic_year_name;
