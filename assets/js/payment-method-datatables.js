    ...
    fv = FormValidation.formValidation(formAddNewRecord, {
      fields: {
        payment_method_name: {
          validators: {
            notEmpty: {
              message: 'The payment method name is required'
            }
          }
        }
      },
...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ajax: {
        url: window.paymentMethodUrls.getData,
        type: 'GET',
        dataSrc: 'data'
      },
      columns: [
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['payment_method_name'];
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Payment Method</h5>');
  }
  ...
  if ($new_name != '') {
    var url = window.paymentMethodUrls.store;
    var method = 'POST';
    var data = {
      _token: $('meta[name="csrf-token"]').attr('content'),
      payment_method_name: $new_name
    };

    if (id) {
      url = window.paymentMethodUrls.update + '/' + id;
      method = 'PUT';
    }
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        offCanvasEl.hide();
        $('#form-add-new-record').removeAttr('data-id');
        $('.add-new-record .dt-full-name').val(''); // reset input
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: window.paymentMethodUrls.destroy + '/' + data.id,
          type: 'DELETE',
          data: {
            _token: $('meta[name="csrf-token"]').attr('content')
          },
          ...
          error: function (error) {
            toastr.error('An error occurred while deleting the payment method.');
          }
        });
      }
    });
  ...
  $('.datatables-basic tbody').on('click', '.item-edit', function () {
    ...
    document.querySelector('.dt-full-name').value = data.payment_method_name;
